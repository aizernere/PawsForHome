...
def dashpets(request):
    ...
    all_pets = Pet.objects.filter(owner=request.user, status=1,is_deleted=False) 
    pet_count = all_pets.count()
    #to do pending request
    adoption_requests = AdoptionRequest.objects.filter(pet__in=all_pets,status = 1)
    pending_count = adoption_requests.count()

    popular_pets = (
        all_pets.annotate(favorites_count=Count('favorite'))
        .filter(favorites_count__gt=0)
        .order_by('-favorites_count')
    )

    pets = all_pets.order_by('-created_at')[:4]
